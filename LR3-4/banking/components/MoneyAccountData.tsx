    ...
    <div className='bg-white overflow-hidden shadow rounded-lg border mt-5'>
      <div className='border-t border-gray-200 px-4 py-5 sm:p-0'>
        ...
        <div className='py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
          ...
          <dd className={textContentClassName}>{account.number} {account.name.trim().length === 0 && 'Not provided'}</dd>
        </div>

        <div className='py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
          <dt className={textHeadingClassName}>Available amount</dt>
          <dd className={textContentClassName}>{account.amount}</dd>
        </div>
      ...
      {<Button type="button" text="Generate this card's QR for fast money receiving" appearance="ordinary" onClick={generateQR}/>}
      {qrData && <QRCode
