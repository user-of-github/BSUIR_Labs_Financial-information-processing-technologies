...
export const Input = <InputValueType extends number | string>({
  ...
  return (
    <input
      type="text"
      className="disabled:pointer-events-none block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:outline-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
      placeholder={placeholder}
      disabled={disabled}
      value={value}
      onChange={event => onChange?.((type === 'number' ? Number(event.currentTarget.value) : event.currentTarget.value) as InputValueType)}
    />
  );
};
